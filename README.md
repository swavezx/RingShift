Driverless kernel memory framework using an SSDT Proxy and PreviousMode Patch — no driver remains loaded at runtime.
...
The BYOVD is used to exploit a patched vulnerability in Windows — specifically the SP1 (Windows Server 2003 Service Pack 1) security patch, which prevents usermode processes from opening protected handles like \Device\PhysicalMemory.
...
After the exploit is complete, the driver is unloaded. To avoid detection, MmUnloadedDrivers cache is spoofed to make the driver appear to have been loaded from a Critical Process, making it invisible to driver scanners.
</details>
<details>
...
A UAC bypass via fodhelper.exe registry hijack is also included for going from user → admin without a prompt.
</details>
<details>
...
Manually walks the x64 page table hierarchy PML4 → PDPT → PD → PT via physical reads to translate arbitrary virtual addresses to physical addresses. Supports 4KB, 2MB, and 1GB pages. Used internally to locate the SSDT entry's physical address for patching.
</details>
<details>
...
Includes a 4-level page table cache (512 entries per level, 64-byte aligned) for fast VA→PA translation of the target process without redundant physical reads.
Exposes a unified memory::read<T> / write API over the entire stack.
</details>
...
KunaiRingShiftBackend\Device\PhysicalMemoryNT Syscall ProxyMethodNtOpenSection → MapViewOfFile (2GB chunks)NtReadVirtualMemory / NtWriteVirtualMemoryPreviousModePatched temporarily to open the section handle, then restoredPatched and kept for the lifetime of the sessionVAD PatchingYes — hides the physical mappings from scannersNot neededPPLSets process as PPL Anti-Malware after setupNot yet implemented
